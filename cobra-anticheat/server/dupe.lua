...
-- Cobra Development
--
-- Pattern: Config is read once into a local 'Cfg' table after the first
-- server tick, then ONLY Cfg is used throughout the file. If Config
-- ever becomes nil (e.g. shared script reload), Cfg remains valid.
-- =============================================

local itemLog         = {}
local moneyLog        = {}
local whitelistActive = {}
local Cfg             = nil   -- populated in init thread
...
for _, evName in ipairs(MONEY_WHITELIST_EVENTS) do
    AddEventHandler(evName, function()
        local src = source
        if src and src ~= 0 then whitelistActive[src] = GetGameTimer() end
    end)
end
...
local function TrackItemAdd(src, itemName, qty)
    ...
    if not Cfg.Detections.itemDupe then return end

    local watched = false
    for _, w in ipairs(Cfg.Economy.watchedItems) do
    ...
    if (now - entry.firstTime) > Cfg.Thresholds.dupeItemWindow then
        entry.qty = 0; entry.firstTime = now; entry.txns = {}
    end

    entry.qty = entry.qty + qty
    entry.txns[#entry.txns + 1] = { qty = qty, time = now }

    if entry.qty >= Cfg.Thresholds.dupeItemMaxQty then
        local detail = ('Item "%s" added ×%d (net +%d) within %dms — possible dupe'):format(
            itemName, #entry.txns, entry.qty, Cfg.Thresholds.dupeItemWindow)
        itemLog[src][itemName] = nil

        SendSuspiciousAlert(src, 'itemDupe', detail)

        if Cfg.Webhooks.economy ~= 'YOUR_WEBHOOK_URL_HERE' then
            PerformHttpRequest(Cfg.Webhooks.economy, function() end, 'POST',
                json.encode({
                    username = Cfg.ServerName .. ' | Cobra Anti-Cheat',
                    embeds = {{
                        title  = '📦 ITEM DUPLICATION DETECTED',
                        color  = 16744272,
                        fields = {
        ...
        local action = Cfg.AutoActions.itemDupe
        if action == 'ban' then
            BanPlayer(src, 'Auto-ban: item dupe ('..itemName..')', nil, 0)
        elseif action == 'kick' then
            KickPlayer(src, 'Auto-kick: item dupe', nil)
        end
    end
end

local function TrackItemRemove(src, itemName, qty)
    if itemLog[src] and itemLog[src][itemName] then
...
local function SnapshotBalance(src)
    if not Cfg then return end
    if not Cfg.Detections.moneyDupe then return end
    local cash, bank = GetPlayerBalance(src)
    moneyLog[src] = { cash = cash, bank = bank }
end
...
local function CheckBalanceSpike(src)
    ...
    local cash, bank = GetPlayerBalance(src)
    if cash < Cfg.Economy.minBalanceToTrack and bank < Cfg.Economy.minBalanceToTrack then return end
    ...
    local function FlagMoney(kind, amount)
        ...
        if Cfg.Webhooks.economy ~= 'YOUR_WEBHOOK_URL_HERE' then
            PerformHttpRequest(Cfg.Webhooks.economy, function() end, 'POST',
                json.encode({
                    username = Cfg.ServerName .. ' | Cobra Anti-Cheat',
                    embeds = {{
                        title  = '💰 MONEY DUPLICATION DETECTED',
                        color  = 15158332,
                        fields = {
                            { name='👤 Player',   value='`'..(GetPlayerName(src) or '?')..'` (ID:'..src..')', inline=true  },
        ...
        local action = Cfg.AutoActions.moneyDupe
        if action == 'ban' then
            BanPlayer(src, 'Auto-ban: money dupe (£'..tostring(amount)..' '..kind..')', nil, 0)
        elseif action == 'kick' then
            KickPlayer(src, 'Auto-kick: money dupe', nil)
        end
    end

    if cashDiff > Cfg.Economy.cashSpikeThreshold then FlagMoney('Cash', cashDiff) end
    if bankDiff > Cfg.Economy.bankSpikeThreshold then FlagMoney('Bank', bankDiff) end
...
AddEventHandler('playerDropped', function()
    local src = source
    itemLog[src] = nil; moneyLog[src] = nil
end)
